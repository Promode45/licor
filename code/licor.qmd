...
resp_data_w <- resp_data_w %>% 
 ...
 mutate_if(is.character, as.numeric) %>% 
  mutate(daym = date(date_time_initial_value)) %>% 
...
resp_data_wrm1$date_time_initial_value <- factor(date(resp_data_wrm1$date_time_initial_value))
resp_data_wrm1 |> 
...
ggplot(resp_data_wrm, aes(x = date_time_initial_value, y = swc_2_mean, shape = treatment)) +
  geom_line(linewidth = 1.2) +  # Adjust line size for better visibility
  geom_point(size = 1.5, alpha = 0.6) +  # Optional: Add points for better visibility of data
  scale_x_datetime(breaks = c("2021-02-01", "2021-04-01"), date_labels = "%b %d", date_breaks = "1 month") +  # Improve date formatting
  labs(title = "Soil Water Content Over Time by Treatment",
       x = "Date",
       y = "Mean Soil Water Content",
       color = "Treatment") +  # Add titles and axis labels
